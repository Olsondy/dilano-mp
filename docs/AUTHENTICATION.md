...
- 小程序端通过 `wx.login()` 获取 `loginCode`
- 用户在我的页面通过 `open-type="getPhoneNumber"` 授权拿到手机号授权 `code`
- 前端调用后端登录接口换取 `access_token`
...
目标是尽早校验当前 token 是否有效。若返回 401，不在 `app.ts` 内显式处理，而是交给 `request.ts` 的统一拦截逻辑。
