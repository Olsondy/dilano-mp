...
- 后端认证/用户接口定义集中在 `miniprogram/api/auth.ts` 与 `miniprogram/api/user.ts`，`AuthService` 只负责编排登录态和 token 写入/清理
...
`bootstrapSession()` 会复用同一个启动 promise。应用首次进入时，如果页面或心跳在静默登录完成前发起业务请求，请求层会等待该 promise 完成后再读取本地 `access_token`，避免旧 token 先触发“会话过期”提示，而 `silent-login` 随后才写入新 token。

## API Module Boundary

认证相关接口按职责拆分：

- `api/auth.ts`：`silentLogin`、`oneClickLogin`、`logout`
- `api/user.ts`：`getUserInfo`、`cancelAccount`

`utils/auth.ts` 中的 `AuthService` 负责调用这些 API 并处理 token，不再直接持有后端接口 URL。
...
- 新增登录相关能力时，接口定义优先扩展 `api/auth.ts` 或 `api/user.ts`，token/session 编排再放到 `AuthService`。
